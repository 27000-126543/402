...
export function generateMockData() {
  ...
  const firstOperator = personnel.find(p => p.role === 'operator')
  if (firstOperator) {
    const fixedSchedules = [
      ...
      },
      {
        ...
        confirmStatusName: '已确认',
        confirmTime: new Date().toISOString().slice(0, 19).replace('T', ' '),
        confirmOperator: firstOperator.name,
        changeoverTime: 35,
        conveyorLoad: 68,
        blockageRisk: 22,
        createTime: new Date().toISOString().slice(0, 19).replace('T', ' '),
      ...
      },
      {
        ...
        approver: '张主管',
        approvalTime: new Date().toISOString().slice(0, 19).replace('T', ' '),
        remarks: '注意检查橡胶中的金属杂质'
      }
    ]
    schedules.unshift(...fixedSchedules)
  }
